...
当前最新可下载版本：**[v0.2.0-alpha（预发布版）](https://github.com/StevenLee147/MyHKU/releases/tag/v0.2.0-alpha)**。按设备选择下表中的安装包，无需下载源码。

| 设备 | 对应安装包 | 最新版本下载 | 安装提示 |
| --- | --- | --- | --- |
| Windows 64 位电脑（Intel / AMD x64） | `MyHKU.Setup.0.2.0-alpha.exe` | [下载安装版 EXE](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/MyHKU.Setup.0.2.0-alpha.exe) | 推荐；运行安装向导。 |
| Windows 64 位电脑（免安装） | `MyHKU.0.2.0-alpha.exe` | [下载便携版 EXE](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/MyHKU.0.2.0-alpha.exe) | 下载后直接运行。 |
| Mac Apple Silicon（M 系列芯片 / arm64） | `MyHKU-0.2.0-alpha-arm64.dmg` | [下载 DMG](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/MyHKU-0.2.0-alpha-arm64.dmg) | 拖入 Applications；如遇系统拦截，见[各设备安装指南](#各设备安装指南)。 |
| Mac Apple Silicon（ZIP 备用包） | `MyHKU-0.2.0-alpha-arm64-mac.zip` | [下载 ZIP](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/MyHKU-0.2.0-alpha-arm64-mac.zip) | 解压后移入 Applications；系统拦截处理见[安装指南](#各设备安装指南)。 |
| Android 8.0 及以上手机 / 平板 | `app-release.apk` | [下载 APK（已签名）](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/app-release.apk) | 下载后打开 APK，按系统提示允许安装此来源的应用。 |
| Android 应用商店分发 / 开发者 | `app-release.aab` | [下载 AAB](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/app-release.aab) | 用于商店分发或生成 APK，不能直接点击安装。 |
...
可下载 [SHA256SUMS.txt](https://github.com/StevenLee147/MyHKU/releases/download/v0.2.0-alpha/SHA256SUMS.txt) 校验文件完整性；`.blockmap`、`latest.yml` 和 `latest-mac.yml` 是更新元数据，无需手动安装。
...
1. 从 **v0.1.4-alpha** 开始，下载固定签名的 **`app-release.apk`**。发布前会检查签名、版本与安装兼容性，并在 Android 8.0 和 Android 15 模拟器验证安装、覆盖升级及启动；不再发布未签名 APK。旧版本中的 `app-debug.apk` 仅用于测试。
2. 在手机中打开 APK，按系统提示为当前浏览器或文件管理器开启「允许来自此来源的应用 / 安装未知应用」，完成安装后可关闭该来源权限。
3. v0.1.4-alpha 及之后的固定签名版本可直接覆盖升级。若已安装 v0.1.1/v0.1.2 的旧测试包，其临时调试签名与新签名不同，Android 会拒绝覆盖并提示「应用未安装 / 与现有软件包冲突」。**卸载旧测试版会清除本地账户、设置和缓存**；请先记录设置、保存所需资料，确认可接受后再卸载旧版并安装新 APK。后续更新请持续使用同一发布渠道。

`app-release-unsigned.apk` 没有签名，不能直接在手机安装；`.aab` 用于商店分发或生成 APK，也不能直接点击安装。普通用户请选择上面的可安装 APK。
