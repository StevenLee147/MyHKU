...
app.whenReady().then(async () => {
  ...
  const legal = await invoke('window.myhkuDesktop.legalStatus()')
  assert.equal(legal.accepted, false)
  assert.equal(visits.portal + visits.sis + visits.moodle, 0, 'no official login before consent')
  await assert.rejects(invoke('window.myhkuDesktop.beginLogin(false)'), /协议/)
  await assert.rejects(invoke('window.myhkuDesktop.acceptLegal("old-version")'), /版本/)
  await invoke(`window.myhkuDesktop.acceptLegal(${JSON.stringify(legal.version)})`)
  assert.equal((await invoke('window.myhkuDesktop.legalStatus()')).accepted, true)
  assert.equal(JSON.parse(readFileSync(join(app.getPath('userData'), 'myhku-legal-consent.json'), 'utf8')).version, legal.version)
  console.log('PASS native agreement gate: no login before consent, stale version rejected, consent persisted')
